<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 min-h-[100px]">
        <div class="relative h-full">
            <x-app.card title="Pendiente de Cobro" amount="$U {{ $receivables->sum('amount') }}"
                description="Total pendiente por cobrar" amountColor="text-green-700" iconColor="text-green-700"
        ...
        <div class="relative h-full">
            <x-app.card title="Cobros" amount="{{ count($receivables) }}"
                description="Transacciones pendientes de cobro" amountColor="text-red-700" amountColor="text-blue-700"
        ...
        <div class="relative h-full">
            <x-app.card title="Pendiente de Pago" amount="$U {{ $payables->sum('amount') }}"
                description="Total pendiente por pagar" amountColor="text-red-700" iconColor="text-red-700"
                icon=' <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
        ...
        <div class="relative h-full">
            <x-app.card title="Pagos" amount="{{ count($payables) }}" description="Transacciones pendientes de pago"
                amountColor="text-blue-700" iconColor="text-blue-700"
                icon='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                         stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                              d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m3.75 9v7.5m2.25-6.466a9.016 9.016 0 0 0-3.461-.203c-.536.072-.974.478-1.021 1.017a4.559 4.559 0 0 0-.018.402c0 .464.336.844.775.994l2.95 1.012c.44.15.775.53.775.994 0 .136-.006.27-.018.402-.047.539-.485.945-1.021 1.017a9.077 9.077 0 0 1-3.461-.203M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/>
    ...
    <div x-data="{ selectedTab: @entangle('selectedTab') }" class="w-full p-4 sm:p-6 mt-4">
        <div x-on:keydown.right.prevent="$focus.wrap().next()" x-on:keydown.left.prevent="$focus.wrap().previous()"
            ...
            <button x-on:click="$wire.selectAll" x-bind:aria-selected="selectedTab === 'all'"
                ...
                x-bind:class="selectedTab === 'all' ?
                    'font-bold text-secondary border-b-2 dark:border-primary-dark dark:text-primary-dark' :
            ...
            <button x-on:click="$wire.selectReceivable" x-bind:aria-selected="selectedTab === 'receivable'"
                ...
                x-bind:class="selectedTab === 'receivable' ?
                    'font-bold text-secondary border-b-2 dark:border-primary-dark dark:text-primary-dark' :
                    'text-on-surface font-medium dark:text-on-surface-dark dark:hover:border-b-outline-dark-strong dark:hover:text-on-surface-dark-strong hover:border-b-2 hover:border-b-outline-strong hover:text-on-surface-strong'"
            ...
            <button x-on:click="$wire.selectPayable" x-bind:aria-selected="selectedTab === 'payable'"
                ...
                x-bind:class="selectedTab === 'payable' ?
                    'font-bold text-secondary border-b-2 dark:border-primary-dark dark:text-primary-dark' :
                    'text-on-surface font-medium dark:text-on-surface-dark dark:hover:border-b-outline-dark-strong dark:hover:text-on-surface-dark-strong hover:border-b-2 hover:border-b-outline-strong hover:text-on-surface-strong'"
                    ...
                    <li wire:key="toggle-{{ $transaction->id }}"
                        ...
                        <div class="flex items-center w-full sm:w-auto mb-2 sm:mb-0">
                            ...
                            <div class="flex-1 space-y-2">
                                <p
                                    class="text-base sm:text-lg font-medium text-on-surface dark:text-on-surface-dark line-clamp-2">
                        ...
                        <div class="flex items-center gap-3 justify-end">
                            <label for="toggle-{{ $transaction->id }}" class="inline-flex items-center gap-3">
                                <input id="toggle-{{ $transaction->id }}" type="checkbox" class="peer sr-only"
                                    role="switch" wire:click.prevent="changeStatus('{{ $transaction->id }}')"
                        ...
                        <div class="flex items-center w-full sm:w-auto gap-2 sm:gap-3 mt-2 sm:mt-0">
